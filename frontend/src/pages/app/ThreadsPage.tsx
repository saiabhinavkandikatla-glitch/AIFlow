  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-semibold">My Threads</h1>
          <p className="mt-2 text-muted-foreground">Search, open, rename, rebuild, or delete the handoffs tied to your account.</p>
        </div>
      ...
      <Card>
        <CardContent className="grid gap-3 p-4 md:grid-cols-[1fr_220px]">
          <div className="relative">
            ...
            <Input className="pl-9" placeholder="Search by title or tag" value={search} onChange={(event) => setSearch(event.target.value)} />
          </div>
          <div className="relative">
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description="Create a new handoff or adjust your search and date filters."
          action={
            <Link to="/app/threads/new" className={buttonVariants()}>
              Create Thread
            </Link>
            ...
            <Card key={thread.id} className="lift">
              <CardContent className="p-5">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => deleteThread(thread)} aria-label="Delete thread">
